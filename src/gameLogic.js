...
function gameStatus(board) {
  ...
  if (
    checkColumnWinner(board) === "r" ||
    checkRowWinner(board) === "r" ||
    checkDiagonalWinner(board) === "r"
  )
    return { status: "winner_red", position: mainCoordenites };

  if (
    checkColumnWinner(board) === "y" ||
    checkRowWinner(board) === "y" ||
    checkDiagonalWinner(board) === "y"
  )
    return { status: "winner_yellow", position: mainCoordenites };
...
function isBoardFull(board) {
  ...
  // filter empty items
  let filteredFlatBoard = flattened.filter(function(item) {
    return item != null;
  });
